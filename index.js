      ...
      if (this.targetDir && typeof this.targetDir === "string") {
        fs.access(this.targetDir, fs.constants.F_OK, async (err) => {
          if (!err) {
            ...
            fs.access(
              ...
              async (err2) => {
                if (!err2) {
                      ...
                      fs.access(
                        ...
                        async (err) => {
                          if (!err) {
                            ...
                            console.log(
                              `所需的autoClickSelect.exe文件不存在，配置请参考文档: https://www.jianshu.com/p/4c854f6c6f86`
                            );
                          }
                          // 点击加载已解压的扩展程序，并选中刚才创建好的目标文件夹
                  ...
                  console.log(
                    "auto-load-chrome-ext-webpack-plugin插件必需的chromedriver.exe文件不存在，配置请参考文档: https://www.jianshu.com/p/4c854f6c6f86"
                  );
                }
              }
            );
            ...
            console.log(
              "auto-load-chrome-ext-webpack-plugin插件必需的targetDir目标文件夹不存在，配置请参考文档: https://www.jianshu.com/p/4c854f6c6f86"
            );
          }
        });
      } else {
        console.log(
          "请正确传入auto-load-chrome-ext-webpack-plugin插件所必需的targetDir参数，配置请参考文档: https://www.jianshu.com/p/4c854f6c6f86"
        );
      }
    });
  }
